namespace Reeb.SqlOM
{
    ...
    public class FromTerm
    {
        ...
        public static FromTerm Function(string functionName, params string[] arguments)
        {
            ...
            return Table(functionCall);
        }

        public static FromTerm Function(string functionName, string alias, params string[] arguments)
        {
            string functionCall = GetFunctionCallString(functionName, arguments);
            return Table(functionCall, alias);
        }

        public static FromTerm Function(string functionName, string alias, string ns, params string[] arguments)
        {
            string functionCall = GetFunctionCallString(functionName, arguments);
            return Table(functionCall, alias, ns);
        }

        public static FromTerm Function(string functionName, string alias, string ns1, string ns2, params string[] arguments)
        {
            string functionCall = GetFunctionCallString(functionName, arguments);
            return Table(functionCall, alias, ns1, ns2);
        }

        private static string GetFunctionCallString(string functionName, params string[] arguments)
        {
            string functionCall = arguments != null
              ? String.Format("{0}({1})", functionName, String.Join(",", arguments))
              : functionName + "()";
            return functionCall;
        }
        ...
        /// Creates a FromTerm which represents a sub-query.
        /// </summary>
        /// <param name="union">A SqlUnion instance representing the sub query</param>
        /// <param name="alias">term alias</param>
        /// <returns>A FromTerm which represents a sub-query.</returns>
        public static FromTerm SubQuery(SqlUnion union, string alias)
        {
            FromTerm term = new FromTerm();
            term.expr = union;
            term.alias = alias;
        ...
        /// <summary>
        /// Render [] around the table name
        /// </summary>
        public bool RenderIdentifierQuotes
        {
            get { return renderIdentifierQuotes; }
            set { renderIdentifierQuotes = value; }
        }
