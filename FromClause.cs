...
public enum JoinType
    ...
    public class FromClause : ICloneable
    {
        ...
        {
            if (joins.Count == 0 && baseTable is not null)
                return string.Compare(baseTable.RefName, alias, StringComparison.OrdinalIgnoreCase) == 0;

            foreach (Join join in joins)
            {
                if (string.Compare(join.RightTable.RefName, alias, StringComparison.OrdinalIgnoreCase) == 0)
                    return true;
            }
        ...
        {
            WhereClause clause = new(WhereClauseRelationship.And);
            foreach (JoinCondition cond in conditions)
            {
                switch (cond.Type)
                {
                    case JoinConditionType.And:
                        clause.Terms.Add(WhereTerm.CreateCompare(SqlExpression.Field(cond.LeftField, leftTable), SqlExpression.Field(cond.RightField, rightTable), CompareOperator.Equal));
                        break;
                    case JoinConditionType.Raw:
                        clause.Terms.Add(WhereTerm.CreateCompare(SqlExpression.Raw(cond.LeftField), SqlExpression.Raw(cond.RightField), CompareOperator.Equal));
                        break;
                    default:
                        throw new InvalidQueryException($"Unknown join condition type: {cond.Type:G}");
                }
            }
        ...
        public bool IsEmpty
        {
            get => baseTable is null && joins.Count == 0;
        }
        ...
        public FromClause Clone()
        {
            FromClause a = new()
            {
                joins = new JoinCollection(joins),
                baseTable = baseTable
            };
            return a;
        }
